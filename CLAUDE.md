...
- **Zatiaľ len vytváranie novej úlohy** — **prepojenie s už existujúcou Caflou úlohou** (napr. keď už úloha „Príprava ASR“ existuje) zatiaľ nie je; pri takejto fáze treba pozor, aby sa nevytvorila duplicita. Overené headless testom s podvrhnutými `fetch`/`sb.from` (žiadny reálny zápis): POST telo, update riadku, prepočty dátumov, viewer vs. editor render. Skutočný zápis do Caflou zatiaľ Jozef netestoval.

**Nedorobené / ďalší krok:** prepojenie s **existujúcou** Caflou úlohou (výber z úloh projektu), synchronizácia termínu aj pri ⚡ plánovaní, prepis existujúcich `requests.podklady_datum` návrhom (zatiaľ len navrhuje, nezapisuje), jednorazové ručné zadanie aktuálnej záťaže tímu pred prvým použitím (cez ručný štart v modáli).
