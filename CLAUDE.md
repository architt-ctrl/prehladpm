...
- **Jemné hranové čiary (architektonický vzhľad, 2026-09-11):** po stabilizácii geometrie sa pre každý `THREE.Mesh` v `model.object` (traverse) pridá `THREE.LineSegments` z `THREE.EdgesGeometry(geom, 25)` (25° prah, tmavá poloprieh. `#2c2825` @ 0.35 opacity) ako jeho child. **Gotcha:** `EdgesGeometry` priamo na Fragments geometrii padá (`Cannot read properties of undefined (reading '0')` v `BufferAttribute.getX`, volané z `EdgesGeometry` konštruktora — Fragments atribúty nie sú štandardné `BufferAttribute`/majú inú internú štruktúru) — treba najprv postaviť čistú `THREE.BufferGeometry` len z holých `position`/`index` typed array polí (`new THREE.BufferAttribute(attr.array, attr.itemSize)`) a na tej spustiť `EdgesGeometry`. Per-mesh try/catch (nie jeden veľký try okolo celého cyklu) — inak jedna zlá geometria zhodí spracovanie všetkých ostatných meshov.
