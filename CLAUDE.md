...
- **Overené** headless testami s podvrhnutými `fetch`/`sb.from` (žiadny reálny zápis): rozvrh (štart cez víkend, polovice dňa), ▲▼ a odhad, viewer bez ovládacích prvkov, celá synchronizácia (nové/existujúce/preradené/zmazané/bez riešiteľa). Reálna tabuľka `plan_ulohy` zatiaľ v Supabase **neexistuje** (bez SQL stránka zobrazí výzvu na jeho spustenie).
