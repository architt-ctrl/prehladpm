...
- **Hrubá rezová čiara (2026-09-11):** `Clipper` sám osebe je len holé WebGL orezanie (`material.clippingPlanes`, žiadny built-in cap/fill/outline) — Jozef chcel architektonický štandard: výraznú hrubú čiaru presne tam, kde rovina reže konštrukciu. `@thatopen/components` na to nemá hotovú funkciu (`EdgeProjector`/`Edges` trieda v knižnici je určená na generovanie 2D "hidden line" výkresov s vizibilitou/BVH — príliš ťažké pre tento účel, ale potvrdilo správny zdroj dát nižšie). Riešenie postavené ručne:
  - Prienik roviny rezu s trojuholníkom = `THREE.Plane.intersectLine()` na každej z 3 hrán trojuholníka, zozbierané 2 unikátne body = jeden úsečkový segment. Beží nad **rovnakými per-položkovými dátami ako hranové čiary vyššie** (`model.getItemsGeometry`, `itemGeomMap` zdieľaná premenná v `ifc-viewer.html`) — nie nad zlúčenou mesh geometriou (rovnaký dôvod ako pri hranách: raw mesh dáta sú v "šablónovom" priestore pred per-instance transformáciou).
  - Skutočná hrúbka čiary (nie len 1px `LineBasicMaterial`, ktorý väčšina GPU/prehliadačov ignoruje) — `Line2`/`LineGeometry`/`LineMaterial` z `three/examples/jsm/lines/` (import mapa, rovnaká `three@0.185.1` verzia ako zvyšok), `linewidth: 3`, treba nastaviť `material.resolution` na veľkosť viewportu a aktualizovať pri `resize`.
  - Prepočítava sa (`refreshCutLines()` — zmaže všetky staré `Line2` objekty, prejde `clipper.list`, pre každú aktívnu rovinu znova spočíta prienik) pri: vytvorení rezu (dvojklik), zmazaní jedného rezu (Delete/Backspace), `deleteAll()`, a pri `planeEntry.onDraggingEnded` (Clipper podporuje ťahanie existujúcej roviny — počas ťahania sa čiara neaktualizuje naživo, len po pustení, kvôli výkonu pri ~800k+ trojuholníkoch).
  - `clipper.list` je Map, každý záznam (`planeEntry`) má `.three` (skutočný `THREE.Plane`, world-space) — `createFromNormalAndCoplanarPoint(world, normal, point)` je užitočný priamy spôsob vytvorenia roviny na testovanie bez potreby raycast-based `create(world)` (ten vyžaduje skutočný klik na geometriu, v headless testoch nespoľahlivé).
