...
- **Reálny priebeh pri preťažení (`harmSimulujRealne`, 2026-07-03):** keď má človek v niektorý deň súbeh prác nad 100 % (vzniká LEN ručne zadanými štartmi — plánovač preťaženie nevytvára), práce sa reálne spomalia a konce posunú za nominálne. Pravidlo delenia kapacity (Jozef): práca s termínom (`prioritny`) sa robí prednostne naplno; ostatné si delia zvyšok pomerne; ak majú termín všetky prekryté (alebo žiadna), delia sa pomerne všetky. Simulácia po dňoch (effort = trvanie×7×alokácia percento-dní), vracia `{priradenia: [{...,realny_koniec,spomalene}], usage: Map('meno|Y-M-D'→%)}`; trvanie vie odvodiť aj z `koniec_datum` ak `trvanie_tyzdne` chýba. `harmNaplanujFrontu` interné prepnuté na dennú kapacitu (`harmJeKapacitaVolnaDni`/`harmNajdiNajskorsiStartDni` nad usage mapou) — plánuje až po reálnych koncoch; reťazenie blokov aj `harmSpustiPlanovanie` (návrhy podkladov) používajú reálne konce. UI: `simById` v harmonogram.html — preškrtnutý nominálny koniec + oranžový „reálne {datum}", badge „spomalené — súbeh prác", timeline bunky nad 100 % červené s tooltipom. Staré intervalové `harmJeKapacitaVolna`/`harmNajdiNajskorsiStart` ostávajú exportované (kompatibilita)
