...
**Výkon — sekvenčné stránkovanie bolo pomalé (2026-09-22, OPRAVA BOLA CHYBNÁ, opravené znova 2026-09-23):** keďže `project_id` filter treba obchádzať prechádzaním celého účtu (~16 strán pri súčasnom objeme úloh), `loadCaflouTasks` pôvodne čakalo na každú stránku `await` **jednu po druhej** — Jozef nahlásil, že otvorenie detailu projektu trvá dlho. Prvá oprava (2026-09-22) natiahla zvyšné stránky **naraz paralelne** (`Promise.all`) — overené len synteticky (fake `fetch` s umelou latenciou), nikdy proti reálnemu Caflou. **Táto oprava bola nesprávna a v produkcii spôsobila presne to, čo Jozef nahlásil 2026-09-23: „niektoré/žiadne úlohy nevidím" + „trvá veľmi dlho".** Skutočná príčina: Caflou stránkovaný zoznam (`GET /tasks?per=100&page=N`) **netoleruje súbežné volania** — pri paralelnom fetchi 15 stránok časť z nich vracala `{"message":"Rate limit exceeded - same request is processing already"}` (overené naostro 2026-09-23 priamym volaním z Node — zlyhávalo už pri súbežnosti len 2). Kód pritom kontroloval len `first.error` na 1. stránke, na zvyšných stránkach chybu vôbec nekontroloval (`d.results || []` na zlyhanej stránke ticho vrátilo `[]`) — úlohy z tej stránky sa tak potichu stratili, bez akejkoľvek chyby v UI. Pri opakovanom otváraní/viacerých projektoch naraz to zároveň spôsobovalo, že rate-limitované requesty sa márne opakovali → vnímané spomalenie.

**Skutočná oprava (2026-09-23):** namiesto stránkovaného zoznamu sa pri známych `proj.caflou_task_ids` (bežný prípad — vyplní `syncData()`/`parseCaflouProject` z `p.task_ids`) použije priamo `GET /tasks/{id}` **pre každé konkrétne ID paralelne** — tento endpoint Caflou paralelizmus **toleruje bez problému** (overené naostro: 3× opakovaný test so 17 súbežnými `GET /tasks/{id}`, 0 zlyhaní, ~0,5–1,2s). Stránkovaný zoznam (sekvenčne, ako pred prvou opravou) ostáva len ako **fallback pre prípad, že projekt nemá žiadne známe `caflou_task_ids`** (zriedkavé — buď projekt reálne nemá úlohy, overené naostro na `24-021`/`342680`, ktorý má v Caflou `task_ids: []` aj po plnom sekvenčnom prejdení celého účtu 1516 úloh, alebo čerstvo vytvorená úloha ktorú `syncData` ešte nezachytil). **Poučenie:** syntetický test s fake `fetch` overí len logiku kódu, nie správanie reálneho API pod záťažou (rate limity, konkurenčné limity) — pri optimalizáciách volajúcich cudzie API naraz vo väčšom počte treba aspoň raz overiť naostro, nie len mockom.

**Výkon — druhé kolo, 4 Supabase dotazy tiež bežali sekvenčne (2026-09-22, ešte ten istý deň):** Jozef nahlásil, že otváranie niekedy trvá dlho aj po oprave vyššie — druhá príčina bola za `caflouTasksCache[cislo] = all;`: 4 nezávislé Supabase dotazy (`task_specialists`, `task_email_threads`, `task_notes`, `requests`→`invitations` na `taskPonukySet`) sa volali `await` jeden za druhým, hoci na sebe navzájom nezávisia (líšia sa len tým, do ktorej cache dopĺňajú dáta). Každý zabalený do vlastnej `async () => { try {...} catch(e){} }()` IIFE (zachováva presne rovnaké tiché-zlyhanie správanie ako predtým, len sa už nečaká na dokončenie predchádzajúceho pred spustením ďalšieho) a spustené naraz cez jeden `await Promise.all([...])`. Overené synteticky (Caflou 2×100ms + 4× Supabase 200ms): pôvodne by to bolo ~1000ms sekvenčne, po oprave 418ms.
...
**Popis úlohy z Caflou, editovateľný (2026-09-18):** Caflou `/tasks` už v bežnom fetchi (`loadCaflouTasks`) vracia aj pole `description` (rich-text HTML, presne to čo je v Caflou na karte úlohy) — netreba extra API volanie na čítanie. Pôvodne (prvá verzia, ešte ten istý deň) bol len read-only náhľad — Jozef chcel priamo aj **písať/upravovať** popis v appke, nie len vidieť, takže prerobené na `<textarea id="tdesc-{editKey}">` v edit forme, hneď nad poľom na názov.
- **HTML ↔ plain text konverzia** (`taskDescToPlainText`/`plainTextToTaskDescHtml`) — popis z Caflou môže byť bohatý HTML (často prilepený z Wordu, s `<o:p>` a pod.), na editáciu v obyčajnej textarei sa najprv "odstriehne" na čistý text (`taskDescToPlainText`: nastaví HTML do dočasného `<div>` a prečíta `.textContent` — bezpečnejšie než regex, mimochodom odstraňuje aj XSS riziko, ktoré malo pôvodné `innerHTML`-zobrazenie). Pri ukladaní sa čistý text jednoducho zabalí späť do `<p>...</p>` s `<br>` namiesto nových riadkov (`plainTextToTaskDescHtml`) — **pôvodné bohaté formátovanie (tučné, odkazy, viacero odsekov) sa needituje ani nezachová 1:1**, len samotný text prežije round-trip. Toto je vedomý kompromis kvôli jednoduchosti, nie bug.
- `saveCaflouTaskEdit` číta `#tdesc-{key}`, cez `plainTextToTaskDescHtml` skonvertuje a pošle v PATCH tele ako `description` (rovnaký `{task: {...}}` vzor ako `name`/`end_time`) — **overené naostro** (2026-09-18): vytvorená dočasná testovacia úloha, PATCH `description`, GET na potvrdenie zhody, úloha hneď zmazaná.

**PENDING: „Spolupracujúci užívatelia" na úlohe (2026-09-23, nič ešte neimplementované):** Jozef si všimol, že Caflou má na úlohe okrem hlavného riešiteľa (`target_user_id`) aj samostatné pole na ďalších ľudí, ktorí na nej pracujú — v appke sa toto zatiaľ nikde nezobrazuje ani needituje. Overené priamo cez API (`GET /tasks/{id}`): pole sa volá **`user_ids`** (pole Caflou `user_id` hodnôt, rovnaký `id` priestor ako `CAFLOU_USERS`/`target_user_id`), oddelené od `target_user_id`. Naživo overené na reálnych dátach — 117 z 1516 úloh má neprázdne `user_ids` (napr. úloha `1244602` „Zameranie": `target_user_id: 49419` (Tomáš) + `user_ids: [50218, 50219]` (Miroslava, Jozef)). Rozsah zmeny (zobrazenie v riadku úlohy / editovateľnosť v edit forme / započítanie do „Int tím" vyťaženia) **zatiaľ nebol s Jozefom prebratý** — čaká na ďalšiu session.
...
**Gotcha — duplicitné DOM prvky (2026-09-24):** `toggleDennikThread` prekreslí vlákno **súčasne** v detaile projektu (`#dlist-{cislo}`, je stále v DOM aj keď je zbalený) aj v modáli Zápisky, takže pole odpovede existuje 2×. Pôvodné `getElementById('ri-…')` v `addDennikReply` našlo vždy prvé (prázdne, v detaile projektu) → odpoveď z modálu sa potichu neuložila („nejde mi potvrdiť a uložiť"). Teraz `addDennikReply(cislo, parentId, src)` berie vstup z prvku, z ktorého prišiel klik/Enter (`this`), a `id` na vstupe je odstránené. **Pravidlo:** pri UI, ktoré sa renderuje na viacerých miestach naraz z jedného zdroja (detail projektu + modál), nikdy nespoliehať na `id` — hľadať vstup relatívne k prvku, ktorý akciu vyvolal.
...
**„+ Priradiť dopyt k existujúcej úlohe" / Dohodnutá spolupráca (`toggleDohodaForm`/`buildDohodaFormHtml`/`saveDohodaSprac`) — retroaktívny zápis pre úlohy spred systému dopytov:** výber úlohy (externé Caflou úlohy projektu bez existujúceho `requests.caflou_task_id`) + profesista, zapíše sa rovno ako `selected` víťaz bez podávania ponuky. **Dropdown profesistu sa obmedzuje na už priradeného (2026-09-14):** `toggleDohodaForm` pri otvorení dotiahne `task_specialists` pre projekt priamo do `extSpecOverride` (nespolieha sa, že bol už predtým otvorený panel úloh projektu — inak by mapa mohla byť prázdna). Po výbere úlohy (`onchange` na `#dohoda-task`) `updateDohodaSpecSelect(taskId)` prepíše `#dohoda-spec`: ak má úloha záznam v `extSpecOverride[taskId]`, dropdown ukáže **len jeho** ("už priradený k úlohe") — dôvod (Jozef): keď úloha už má zaznamenaného profesistu, nedáva zmysel ponúkať na výber hocikoho iného. Ak úloha žiadneho zaznamenaného profesistu nemá, dropdown ostáva plný zoznam (nič na obmedzenie).
...
- `koordIconHtml(cislo)` — ak je `cislo` v mape → modrý `🔗` odkaz priamo na Drive (`target="_blank"`); inak `🔍` tlačidlo. **Klik zároveň skopíruje link do schránky (2026-09-14)** — pôvodne len otváral priečinok v novej karte, Jozef potreboval link priamo poslať profesistovi (nie len si ho sám pozrieť), takže `onclick` teraz popri navigácii aj `navigator.clipboard.writeText(url)` + toast. Appka pri tomto linku nemení Drive zdieľanie (na rozdiel od IFC modelu nižšie) — funguje, lebo tento istý link (`requests.folder_url`) sa už profesistom bežne posiela cez `portal.html`, takže zdieľanie priečinka je na strane Drive už vyriešené.
...
### Plánovanie práce kolegov (`planovanie.html`, 2026-09-21)

**Prečo vzniklo:** Jozef uznal, že rozdelenie na fázy/bloky s alokáciou v `harmonogram.html` je zbytočne zložité — reálna potreba je **jednoduchá**: kolegovia už majú v Caflou zadané úlohy a treba určiť, **čo budú kedy robiť** (poradie + odhad). Začalo sa "od piky" novou stránkou; starý `harmonogram.html` (fázy, ⚡ automatický plánovač, prideľovanie) **zatiaľ ostáva nedotknutý** — nerozhodnuté, či sa zruší.

**Rozhodnutia (Jozef):** (1) plánuje sa **poradie + odhad dní**, nie voľné ťahanie pruhov; (2) plánujú sa **len úlohy s tagom `harmon`**; (3) má to byť **jednoduché a prehľadné**; (4) plán vidia všetci kolegovia, upravuje len prihlásený (rovnaký model ako harmonogram: zobrazenie bez prihlásenia, `isEditor`/`body.is-editor`, magic link, UI-only ochrana).

**Ako to funguje:**
- **Tag:** v prehľade projektov je v edit forme úlohy tlačidlo **📅 Plánovať** (pridá/odoberie tag `harmon`), úloha s tagom má v riadku ikonu 📅. **Dôležité:** `saveCaflouTaskEdit`, `bulkSetFaza` aj `bulkSetExtInt` predtým prepisovali `tags` len z `ext` + fáza-tagu, čím by každé uloženie úlohy potichu zmazalo `harmon` (a akýkoľvek iný cudzí tag) — teraz sa cez `otherTaskTags(t)` zachovávajú všetky tagy okrem `ext`/fáza-tagov.
- **Tabuľka `plan_ulohy`** (SQL `supabase/plan-ulohy-setup.sql`, **treba spustiť ručne v Supabase SQL Editore**): `task_id` (PK, Caflou ID), `projektant`, `poradie`, `odhad_dni` (= **zostáva** pracovných dní) a **kópia** z Caflou (`nazov`, `cislo`, `projekt`, `termin`), aby plán videli aj kolegovia bez Caflou prístupu v prehliadači (Caflou kľúč je len v localStorage). Vlastné dáta plánu sú len poradie a odhad.
- **Tlačidlo „↻ Načítať z Caflou“** (`syncFromCaflou`, len editor s Caflou prístupom `pmCfg3`): prejde všetky Caflou úlohy (`/tasks?per=100`, ~16 strán) a projekty, vyberie `!trash && !finished && tags∋harmon` s riešiteľom z tímu. Existujúce zachová (poradie aj odhad), nové pridá na **koniec radu riešiteľa s odhadom 1 deň**, pri zmene riešiteľa v Caflou úlohu presunie na koniec radu nového kolegu, zmiznuté/dokončené/odtagované zmaže a úlohy **bez riešiteľa** vypíše v stavovom riadku (nezaraďuje). Pri prázdnom pláne sa spustí samo (raz, `autoSyncTried`, aby nevznikla slučka).
- **Rozvrh (`layoutQueue`/`advance`/`skipWeekend`):** os začína pondelkom aktuálneho týždňa (deň = index, `index % 7 >= 5` je víkend); pre každého kolegu sa úlohy skladajú **za sebou od dnes** v poradí `poradie`, práca sa počíta len v pracovných dňoch (víkendy sa preskakujú, sviatky **nie**), odhad môže byť aj 0,5 dňa. Pruhy jedného kolegu sa preto **nikdy neprekrývajú** (žiadne lane-y ako v starom harmonograme). `odhad_dni` je „zostáva“ — bez sledovania postupu ho treba ručne znižovať, inak sa rad každý deň posúva od nového „dnes“.
- **Nestíha termín:** ak posledný deň práce > `termin` z Caflou, pruh má červený obrys a ⚠, v zozname „termín … · ⚠ o N d neskôr“ a pri kolegovi odznak „N nestíha termín“. Farby projektov sú zámerne v odtieňoch 40–320 ° (nie červené), aby sa nepomýlili s výstrahou.
- **UI:** stavový riadok (počet úloh, kedy naposledy z Caflou), **Časová os** (riadok na kolegu, dnešok červená čiara, víkendy šedé; hlavička s pevnými šírkami `flex:0 0 auto` — rovnaké poučenie ako pri harmonograme) a **Poradie a odhady** (karta na kolegu: ▲▼ na zmenu poradia = `moveTask` prečísluje rad, políčko „zostáva N dní“ = `setOdhad`, plánované dátumy, termín). Kolegovia vidia všetko okrem ovládacích prvkov.
...
- **Overené** headless testami s podvrhnutými `fetch`/`sb.from` (žiadny reálny zápis): rozvrh (štart cez víkend, polovice dňa), ▲▼ a odhad, viewer bez ovládacích prvkov, celá synchronizácia (nové/existujúce/preradené/zmazané/bez riešiteľa). Tabuľka `plan_ulohy` je v Supabase spustená (Jozef, 2026-09-21) a overená naostro throwaway insert/delete probom.

**Odznak 📅 pri projekte v `index.html` (2026-09-22):** Jozef chcel hneď vidieť, ktorý projekt má úlohu v Plánovaní práce, nielen pri rozkliknutí detailu (tam to už ukazuje ikona 📅 v riadku úlohy, `isHarmon`/`toggleTaskHarmonBtn`), ale aj v samotnom zbalenom riadku projektu. `planBadgeSet` (`{cislo,...}`) sa **bulk-loaduje v `syncData()`** priamo z `plan_ulohy.cislo` (jeden `select('cislo')`, žiadny scan Caflou úloh) — je to teda odraz posledného „↻ Načítať z Caflou“ na `planovanie.html`, nie live stav. `planBadgeHtml(cislo)` je zdieľaný helper, pridaný na **všetky 3 miesta šablóny riadku projektu** (`projRowHtml`, vyhľadávanie, Archív v `renderProjects()`) — všimnuté pri tejto príležitosti, že existujúce odznaky `hasDennikBadge`/`hasPonukyBadge` boli predtým len v `projRowHtml`, chýbali vo vyhľadávaní aj Archíve (nedotýkané, mimo rozsahu tejto zmeny, len poznamenané pre budúcnosť). Na rozdiel od dennik/ponuky odznakov (ktoré zmiznú pri otvorení detailu — "videl si to") tento **nezmizne pri otvorení** — je to trvalý stavový indikátor "má úlohu v pláne", nie notifikácia o niečom novom.
...
- **Hotová úloha sa v pláne nepočíta (2026-09-25, Jozef: „keď je marknutá ako hotová, prečo píšeš že nestíha?"):** plán pôvodne vyraďoval len `finished=true`, ale stav **„Hotové" ≠ ukončená** (úloha ostáva aktívna, pozri „Important distinction"), takže hotová úloha ďalej zaberala čas v rade a mohla svietiť ako „nestíha". Teraz: (1) `syncFromCaflou` v `planovanie.html` vynecháva aj úlohy so `task_status_name === 'Hotové'`; (2) v `index.html` pri zmene stavu na „Hotové" (`setCaflouTaskStatus`) aj pri ✓ (`finishCaflouTask`) volá `dropDoneTaskFromPlan(cislo, task_id)` — zmaže riadok z `plan_ulohy`, prepočíta `computePlanStatus(planRowsCache)` (`planRowsCache` = riadky z posledného `syncData`) a hneď prekreslí P pri projekte (span `.plan-p` v `.prow-name`). Úloha v stave Hotové má v riadku sivé P „v pláne sa nepočíta". **Návrat zo stavu Hotové** úlohu do plánu automaticky nevráti — pribudne až pri ďalšom „↻ Načítať z Caflou" (na koniec radu, odhad 1 deň). Už existujúce hotové úlohy v `plan_ulohy` zmiznú tiež až po jednom „↻ Načítať z Caflou". **Stav „Akceptované" sa zámerne stále počíta ako rozrobená práca** (nie je to hotovo). Overené headless testom (projekt červený → po „Hotové" zelený, riadok zmazaný).
...
**Harmonogram viditeľný pre kolegov + prideľovanie ako Caflou úloha (2026-09-21):** Jozef chcel, aby harmonogram videl každý kolega (on ho spravuje, kolegovia len vidia) a aby sa veci zo zoznamu čakajúcich dali rovno prideliť kolegovi ako úlohy s termínom, prepojené s reálnou Caflou úlohou (`caflou_task_id`).
- **Zobrazenie bez prihlásenia:** `harmonogram.html` už nemá prihlasovaciu bránu na vstupe — načíta sa pre každého (RLS na `harmonogram` je otvorená, čítanie ide s anon kľúčom). Prihlásenie (magic link, ako predtým) len zapne **editor režim** (`isEditor`, `setEditor()`, trieda `body.is-editor`): tlačidlá „+ Pridať fázu“, „⚡ Naplánovať“, ✎/↺/✕, ▶/⏸ a „Prideliť ako úlohu“ sú `.editor-only` a funkcie majú aj guard `if (!isEditor) return`. Kolegovia vidia tlačidlo „🔒 Prihlásiť (úpravy)“. **Je to ochrana len na úrovni UI, nie autorizácia** — RLS je otvorená (rovnako ako pri `dennik` a ostatných tabuľkách), kto pozná anon kľúč, vie zapisovať. Poznámky k fázam (`poznamka`) a termíny klienta sú teraz viditeľné aj kolegom.
- **Názvy projektov pre kolegov:** bez Caflou prístupu v prehliadači (`pmCfg3` v localStorage) by kolegovia videli len čísla projektov. Preto sa názov ukladá do nového stĺpca `harmonogram.nazov` (SQL: `supabase/harmonogram-nazov-setup.sql`, **treba spustiť ručne v Supabase SQL Editore**); pri uložení fázy sa zapíše, a editor s Caflou prístupom ho doplní spätne pre všetky existujúce riadky (`backfillNazvy()`, po načítaní `caflouNames`). `projLabel()` berie `caflouNames[cislo] || rowNames[cislo]`. Kód zapisuje `nazov` **len ak stĺpec existuje** (`hasNazovCol` = kľúč `nazov` je v načítaných riadkoch) — inak by insert/update pred migráciou zlyhal.
- **Prideliť ako úlohu (`openTaskModal`/`saveTaskAssign`):** tlačidlo „📌 Prideliť ako úlohu“ pri riadku (čakajúcom aj naplánovanom, kým nemá `caflou_task_id`). Modal: názov úlohy (default „Fáza – podpodfáza“), kto (`HARM_PROJEKTANTI`), trvanie, začiatok a termín — **začiatok/trvanie prepočítavajú termín, zmena termínu posúva začiatok** (`taskCalcEnd`/`taskCalcStart`, dni sa posúvajú cez zložky dátumu, nie milisekundy, kvôli zmene letného času). Uloženie: `POST /tasks` (`project_id` z `caflouProjects` podľa `order_number`, `task_status_id` „Nové“, `target_user_id` z `CAFLOU_USER_IDS`, `end_time` = termín `T17:00:00+02:00`, tag fázy `taskFazaTag` — `DSP/PS` sa rozlíši podľa `project_status_name` projektu `3_DSP`→`DSP`, inak `PS`) → potom update riadku v Supabase: `projektant`, `start_datum`, `trvanie_tyzdne`, `pripravene_pokracovat=true`, `caflou_task_id` — vec sa tým presunie z čakajúcich do naplánovaných a do Gantta. Ak Caflou úloha vznikne, ale zápis do Supabase zlyhá, toast uvedie ID úlohy (aby sa pri opakovaní nevytvárala duplicita naslepo). Riadok s `caflou_task_id` ukazuje kolegom aj Jozefovi odkaz „📌 úloha v Caflou“ (`https://app.caflou.com/tornyos/tasks/{id}`).
- **Synchronizácia pri úprave (`syncLinkedTask`):** po uložení úpravy bloku (`saveFaza`) sa pri prepojenej úlohe do Caflou pošle `PATCH` s novým riešiteľom a termínom (reálny koniec zo simulácie, ak je práca spomalená). Best-effort, chyba len zobrazí toast. **Nesynchronizuje sa** pri automatickom plánovaní (⚡), zrušení naplánovania ani zmazaní riadku (Caflou úloha v tom prípade ostáva).
- **Prepojenie s už existujúcou Caflou úlohou (2026-09-21, hneď v ten istý deň ako prideľovanie):** modal „Prideliť ako úlohu“ má prepínač **🆕 nová úloha / 🔗 existujúca úloha** (`syncTaskMode`). Pri existujúcej sa z Caflou načítajú úlohy projektu (`loadProjectTasks`: `task_ids` z API projektov + prechádzanie `/tasks?per=100&page=N`, kým sa nenájdu všetky ID — filter `project_id` server ignoruje; výsledok sa cachuje v `projTasksCache[cislo]` a po vytvorení novej úlohy sa cache pre projekt zahodí) a ponúknu sa len **interné (bez tagu `ext`), nedokončené, nezmazané a ešte neprepojené s inou fázou** úlohy. Výber úlohy predvyplní riešiteľa a termín z Caflou (`taskPickExisting`), dá sa ešte upraviť. Uloženie **nevytvára nič nové**: `PATCH` existujúcej úlohy (`target_user_id` + `end_time` — **riešiteľ a termín úlohy v Caflou sa prepíšu podľa formulára**, modal na to upozorňuje) a update riadku harmonogramu s `caflou_task_id`. K prepojenému riadku pribudlo tlačidlo „odpojiť“ (`unlinkTask`, len nuluje `caflou_task_id`, úloha v Caflou ostáva).
- **Oprava pri tejto príležitosti:** `loadCaflouProjects()` v `harmonogram.html` prestával stránkovať podľa počtu **prefiltrovaných** projektov (`items.length < 100` po vyhodení košov/šablón), takže 2. stránka projektov (Caflou ich má > 100) sa pri akomkoľvek koši na 1. stránke nikdy nenačítala — časť projektov chýbala v názvoch aj v hľadaní. Teraz sa stránkuje podľa surovej odpovede (`d.results.length` / `total_pages`).
- Overené headless testom s podvrhnutými `fetch`/`sb.from` (žiadny reálny zápis): POST telo, PATCH existujúcej úlohy, filtrovanie ponuky úloh, update riadku, prepočty dátumov, viewer vs. editor render. `task_ids` v odpovedi projektov a stránkové prechádzanie úloh overené naostro read-only. Skutočný zápis do Caflou zatiaľ Jozef netestoval. SQL `harmonogram-nazov-setup.sql` je spustené (Jozef, 2026-09-21).

**Gantt — hlavička sa zmršťovala (oprava 2026-09-21):** mesiace a dni v hlavičke časovej osi (`.gantt-head-days` → `.gantt-months`/`.gantt-days`, `display:flex`) mali šírku len viditeľného okna, lebo rodič nemal explicitnú šírku, a flex položky sa `flex-shrink` zmrštili — všetkých ~250 dní sa stlačilo do prvých ~1100 px obsahu (Jozef: "všetko je zhutnené v prvej polovici časovej osi a ďalej nepokračuje"; pri odscrollovanom grafe bola hlavička úplne mimo záberu, takže os vyzerala prázdna). Opravené: `.gantt-month,.gantt-day{flex:0 0 auto}` + explicitná šírka `totalDays*DW` na `.gantt-head-days` aj `.gantt-body`. Pri tej príležitosti: os vždy pokračuje aspoň 120 dní za dnešok (predtým len 14 dní za najneskorším koncom, takže pri všetkých ukončených prácach končila hneď po dnešku) a ak nie je nič naplánované dopredu, prvé zobrazenie ukáže dnešok vpravo (75 %), nie vľavo (33 %), aby nebolo celé prázdne. **Poučenie:** flex kontajner s deťmi s pevnou šírkou väčšou než kontajner vždy potrebuje `flex-shrink:0` (alebo explicitnú šírku rodiča), inak sa deti potichu zmrštia.

**Nedorobené / ďalší krok:** synchronizácia termínu aj pri ⚡ plánovaní, prepis existujúcich `requests.podklady_datum` návrhom (zatiaľ len navrhuje, nezapisuje), jednorazové ručné zadanie aktuálnej záťaže tímu pred prvým použitím (cez ručný štart v modáli).
